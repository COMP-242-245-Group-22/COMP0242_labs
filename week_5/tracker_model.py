...
class TrackerModel:
    ...
    def propagation_model_tracker_fixed_std(obj):
        ...
        S_bar = np.zeros((obj.q * obj.N, obj.m * obj.N))
        T_bar = np.zeros((obj.q * obj.N, obj.n))
        Q_bar = np.zeros((obj.q * obj.N, obj.q * obj.N))
        R_bar = np.zeros((obj.m * obj.N, obj.m * obj.N))

        # Loop to calculate matrices
        for k in range(1, obj.N + 1):
            for j in range(1, k + 1):
                idx_row_S = slice(obj.q * (k-1), obj.q * k)
                idx_col_S = slice(obj.m * (k-j), obj.m * (k-j+1))
                S_bar[idx_row_S, idx_col_S] = obj.C @ np.linalg.matrix_power(obj.A, j-1) @ obj.B

            idx_row_T = slice(obj.q * (k-1), obj.q * k)
            T_bar[idx_row_T, :] = obj.C @ np.linalg.matrix_power(obj.A, k)

            idx_row_col_QB = slice(obj.q * (k-1), obj.q * k)
            Q_bar[idx_row_col_QB, idx_row_col_QB] = obj.Q
    ...
    def computesolution(self,x_ref, delta_x, x_cur, u_shape, H, F_tra):
        ...
        x0 = np.array([1] * u_shape)  # Assuming u_cur has the correct dimension
