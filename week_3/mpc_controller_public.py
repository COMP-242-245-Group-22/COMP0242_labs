...
from simulation_and_control import MotorCommands, PinWrapper, dyn_cancel, pb
...
def getSystemMatrices(sim, num_joints, damping_coefficients=None):
    ...
    if damping_coefficients is not None:
        ...
        A[num_joints:, num_joints:] -= damping_matrix * time_step

    B = np.eye(num_states, num_controls, k=-num_controls) * time_step
...
def main():
    ...
    regulator = RegulatorModel(
        A, B, C, Q, R, N_mpc, num_states, num_joints, num_states
    )
    ...
    for i in range(num_joints):
        ...
        plt.subplot(2, 1, 1)
        plt.plot(
            [q[i] for q in q_mes_all], label=f"Measured Position - Joint {i+1}"
        )
        ...
        plt.subplot(2, 1, 2)
        plt.plot(
            [qd[i] for qd in qd_mes_all],
            label=f"Measured Velocity - Joint {i+1}",
        )
